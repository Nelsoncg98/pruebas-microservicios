...
try {
    ...
    # 3. Process 7 Core
    $pCat = Start-Mvn "Ms-TipoAnalisis (Catalogo)" "ms-tipoanalisis"; $global:pidsList += $pCat.Id
    $pAna = Start-Mvn "Ms-Analisis (Core)" "ms-analisis"; $global:pidsList += $pAna.Id
    $pDet = Start-Mvn "Ms-DetalleAnalisis (Detalle)" "ms-detalleanalisis"; $global:pidsList += $pDet.Id
    Start-Sleep -Seconds 10

    # 4. Action & Orchestration
    $pAct = Start-Mvn "Ms-AgregarTipo (Accion)" "ms-agregartipo"; $global:pidsList += $pAct.Id
    $pGes = Start-Mvn "Ms-GestionAnalisis (Facade)" "ms-gestionanalisis"; $global:pidsList += $pGes.Id

    Write-Host "[ok] Proceso 7 launched. Processes are running in background windows." -ForegroundColor Green
